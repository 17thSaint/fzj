using Statistics
dataloc = "../cluster-data/chemical-potential/data-2023-10-25_15-08/"
...
nrgs = [[parse(Float64,join(split(split(alllines[j][12 + 3*i]," ")[end],".")[1:2],".")) for i in 4:5] for j in 1:length(alllines)]
for (stl,l) in nrgs
	percent_change = abs(stl - l)/abs(l)
	if percent_change > 0.01
		println("Not Converged: ",round(100*percent_change,digits=3)," %")
	end
end
#=
