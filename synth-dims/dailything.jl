...
for i in 1:alpha_count
	...
	alph = alpha_vals[i]
	psi_mid = run_mps_new_variable(seed_data[1]["mps"],seed_data[2],Dict([("phi",2*pi*alph),("typechange","nothing")]))
	psi_m,psi_p = varied_alpha_wavefuncs(psi_mid,merge(seed_data[2],Dict([("phi",2*pi*alph)])),change)
