...
function make_HH_ham(net::TTN.AbstractNetwork; kwargs...)
    ...
    tpo = TTN.TPO(ham,TTN.physical_lattice(net))
...
function build_network(model::String,lx::Int,ly::Int; kwargs...)
    ...
    if model == "HH"
        net = BinaryRectangularNetwork((lx,ly), "Boson"; conserve_qns=true, dim = max_occ+1)
    elseif model == "J1J2"
        net = BinaryRectangularNetwork((lx,ly), "Qubit"; conserve_qns=true)
    end
    return net
end
...
function run_benchmark(pu_type::String,mdim::Int,net::TTN.AbstractNetwork,ham_tpo::TTN.TPO,filepath::Union{String,Nothing}; kwargs...)
    ...
    num_sweeps::Int = get(kwargs,:num_sweeps,20)
    ...
    # do DMRG
    time_start = time()
    sp = TTN.dmrg(psi,ham_tpo; observer=obs, number_of_sweeps=num_sweeps, maxdims=mdim, cutoff=0, eigsolve_krylovdim=5, eigsolve_verbosity=0)
