...
# ╔═╡ ea63bf9a-9360-11ee-3ee1-4935f26717ad
begin
	using ITensors
	using Plots
	using LinearAlgebra
...
# ╔═╡ e1e78c35-daf4-4496-a6f9-0d6fc7a83de5
function hamiltonian_sp(N;L, Φ)
	gj = exp(im*Φ/L)
	gv = fill(gj, L - 1)
	H = Matrix(Tridiagonal(gv, zeros(ComplexF64, L), conj(gv)))
	H[1,end] = (-1)^N * gj
	H[end,1] = (-1)^N * conj(gj)
	H
end

# ╔═╡ ab2b06c3-4dbb-49f3-a4be-df0025ba8a19
function hamiltonian_sp_gauged(N;L, Φ)
	g = exp(im*Φ)
	gv = ComplexF64.(fill(1, L - 1))
	H = Matrix(Tridiagonal(gv, zeros(ComplexF64, L), conj(gv)))
	H[1,end] = -(-1)^N * g
	H[end,1] = -(-1)^N * conj(g)
	H
...
# ╔═╡ a898de35-92ad-4b60-a93d-3791b3a09f04
function hamiltonian(;L,Φ)
	ampo = OpSum()
	gj = exp(im*Φ/L)
	for jj in 1:L
		ampo += (gj, "S+", jj ,"S-", mod1(jj+1, L))
		ampo += (conj(gj), "S+", mod1(jj+1, L), "S-", jj)
	end

	return ampo
end

# ╔═╡ 3cbff274-4a3b-4010-9cda-d8832cac30d6
function translation_operator(L, Φ, N)
	T = ComplexF64.(Matrix(Tridiagonal(ones(L-1), zeros(L), zeros(L-1))))
	g = exp(im*Φ)
	T[1,end] = -(-1)^N * g
	T
end

# ╔═╡ 126b31f2-118b-4cd6-b43c-03f030aaca48
function momenta(;L, Φ, N)
	n = 0:L-1
	return (π*(N+1) - Φ)/L .+ 2π.*(n) ./ L
end

# ╔═╡ abfa2aa4-e2f0-4c9c-9fb3-18b41643c5f6
function current_operator(L, j)
	Jⁱⁿ = zeros(L,L)
	if 1<j<L
		Jⁱⁿ[j, j+1] = -1
	else
	...
	return im*Jⁱⁿ, -im*transpose(Jⁱⁿ)
end

# ╔═╡ f2131efa-8134-4f19-a97f-9aaa7ce5c67a
let
	L = 20
	α = 0.
	Φ = α*2π
	...
	pl = plot()
	H = hamiltonian_sp_gauged(N; L, Φ)
	
	vals, vecs = eigen(H)
	
	#T = translation_operator(L, Φ, N)
	#β = angle.(diag((vecs' *T*vecs))) .+ π
	#scatter!(β, vals, marker = :diamond)
	...
	#pos = 6
	#Jⁱⁿ,Jᵒᵘᵗ = current_operator(L, pos)
	#v₁ = vecs[:,1]
	#inc = imag(dot(v₁, Jⁱⁿ * v₁))
	#outc = imag(dot(v₁, Jᵒᵘᵗ * v₁))
	#k = angle(β)
	#pl
	
end

# ╔═╡ 758da898-71e2-4862-9937-1f7917c0a2b8
begin
	L = 10
	s = siteinds("Qubit", L; conserve_number = true)

	initfn = n -> isodd(n) ? "1" : "0"
	initfn = n -> n∈[1,2,3] ? "1" : "0"

	

	ψ₀ = randomMPS(s, initfn)
	...
	for α in αv
		...
		Φ = α*2π
		H = MPO(hamiltonian(;L, Φ), s)
		E, ψ = dmrg(H, ψ₀; maxdim, noise, nsweeps)

		push!(en, E)
		
		#pos = L÷2
		#g = exp(im*Φ)
		#Jₓ =  g*im*op(s[pos], "S+") * op(s[pos+1], "S-")
		#	 -conj(g)*im*op(s[pos], "S-") * op(s[pos+1], "S+")
		#inc = inner(ψ, apply(Jₓ,ψ))
		#push!(jc, inc)
		push!(ψv, ψ)
	end
...
# ╔═╡ 54c6dfca-8a3c-4fde-9e7f-d6d1d6fbe8e5
begin
	
	pos = L÷2
	jc = ComplexF64[]
	for (j, α) in enumerate(αv)
		ψ = ψv[j]
		Φ = α*2π
		g = exp(im*Φ/L)
		Jₓ =  g*im*op(s[pos], "S+") * op(s[mod1(pos+1, L)], "S-")-
			  conj(g)*im*op(s[pos], "S-") * op(s[mod1(pos+1,L)], "S+")

		#corr_sp = correlation_matrix(ψ, "S+", "S-")

		#Jx = corr_sp[pos, pos+1]
		#@info Jx*im*g
...
# ╔═╡ 7b12c838-98bb-4a3a-8fed-5dc72095771f
let
	pl = plot()
	scatter!(pl, αv, real.(jc))
	∂ϵ = (en[2:end] .- en[1:end-1])/step(αv)/2π

	
	scatter!(pl, αv[1:end-1], ∂ϵ, label = "∂ϵ")
	xlabel!("α/Φ₀")
	#ylims!((-2,-1.8))
end

# ╔═╡ c8d7b257-35c4-4463-8bc8-ca19d2dcea5e
let
	L = 20
	N = 1
	...
	for α in αv
		...
		push!(en, sum(vals[1:N])/N)
	end

	pl = plot()
	#scatter!(pl, αv, real.(jci .+ jco), label = "Jₓ")
	#scatter!(pl, αv, real.(jc))
	scatter!(pl, αv, en .- minimum(en))
...
# ╔═╡ 0bda8a1b-dbc2-4a82-9de6-e2c2dbe22ada
let
	L = 50
	Nv = 0:L
	...
	scatter!(pl, nv, Dₓ, label = false)
	f = x -> 7/π * (1 + cos(2π*x - π))
...
julia_version = "1.9.4"
manifest_format = "2.0"
project_hash = "abd117e6d695dd164b2c16e45b1d3a759a3bee9b"
...
git-tree-sha1 = "cc6847cd0e6fd68226d70d99124001d7e37b5b20"
...
[[deps.NDTensors]]
deps = ["Adapt", "BlockArrays", "Compat", "Dictionaries", "FLoops", "Folds", "Functors", "GPUArraysCore", "HDF5", "InlineStrings", "LinearAlgebra", "PackageExtensionCompat", "Random", "SimpleTraits", "SparseArrays", "SplitApplyCombine", "StaticArrays", "Strided", "StridedViews", "TimerOutputs", "TupleTools", "VectorInterface"]
git-tree-sha1 = "c68878e99fae39c5d4f56e62035b2b9749bda0bf"
