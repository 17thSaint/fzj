...
#= check what ulr-length stuff needs rerunning
if true
    intstren = 4.0

    fullrerun_dict = Dict([("14",[]),("12",[]),("10",[])])
    secondrun_dict = Dict([("14",[]),("12",[]),("10",[])])
    secondrerun_dict = Dict([("14",[]),("12",[]),("10",[])])
        ...
        for f in all_files
            ...
            if haskey(m,"fourpt_momentum")
                ...
                if haskey(m,"fourpt_momentum_1")
                    ...
                    if haskey(m,"observer_1")
                        if abs(m["observer_1"].nrg[end-1] - m["observer_1"].nrg[end]) > m["nrgtol"]
                            append!(fullrerun_dict[string(lx)], m["corr_length"])
                        end
                    else
                        display(m["fourpt_momentum_1"])
                        display(keys(d))
                    end
                else
                    #fourpt1_includedsubset = fourpt1
                    #println("Warning: only one four-point correlator found for file $f, using it without averaging")
                    if abs(m["observer"].nrg[end-1] - m["observer"].nrg[end]) > m["nrgtol"]
                        append!(fullrerun_dict[string(lx)], m["corr_length"])
                    else
                        append!(secondrun_dict[string(lx)], m["corr_length"])
                    end
                end
    ...
    println("Full reruns are: ")
    display(fullrerun_dict)

    println("Run for second states are: ")
    display(secondrun_dict)

    println("Reruns for second states are: ")
    display(secondrerun_dict)
...
if false
    ...
    stren = 4.0
    flux = 0.21
    params_dict = Dict([("output_level",1),("if_pfaffian",true),("lr",0),("hopping_anisotropy",1.0),("Lx",lx),("Ly",ly),("es_count",0),("expander_fraction",1e-5),("particles",n),("mdim",400),("if_save_data",false),("alpha",flux),("if_check_fluxes",false),("if_find_data",false),("onsite_strength",stren),("if_periodic_phys",false),("if_periodic_synth",false)])
    all_states, hamilt, all_obs, all_densmats, all_runtimes = run_synth_dims_generic(params_dict)

end=#

# alberto padding zeroes DMRG testing
# Product State, No conserve, increase zeros = works
# Product State, No conserve, increase randn = works
# Product State, Yes conserve, increase randn = fluxes not equal
# Product State, Yes conserve, increase zeros = works
