...
function get_params_dict_from_filename(filename_before)
	...
	elseif split(filename,"-")[1] in ["mps","ttn","rfa","laugh","basis","ed","wavefuncmps","wavefuncttn","hopping","interaction","periodicpotential","disorder","tevo","wavefunctevo"]
	...
	h5open(file_name,"r") do f
		...
		for datum_key in keys(f["all_data"])
			if occursin("ttn",datum_key)
				data[datum_key] = read(g_alldata, datum_key, TTN.TreeTensorNetwork)
			else
		...
		for metadatum_key in keys(f["metadata"])
			...
			elseif occursin("ham",metadatum_key)
				metadata[metadatum_key] = read(g_metadata,metadatum_key, TTN.ITensorMPS.Sum{TTN.ITensorMPS.Scaled{ComplexF64, TTN.ITensorMPS.Prod{TTN.ITensorMPS.Op}}})
			elseif occursin("expander",metadatum_key)
				metadata[metadatum_key] = read(g_metadata, metadatum_key, TTN.DefaultExpander)
			else
